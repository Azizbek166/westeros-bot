...
async def show_dragon_hub(target, user_id: int, is_message: bool):
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Ajdar mavjud
        stage_names = {
            "egg": "🥚 Ajdar Tuxumi (Hali ochilmagan)",
            "baby": "🐉 Kichik Ajdar (Olov purkay oladi)",
            "adult": "🦅 Bahaybat Jangovar Ajdar (Qal'alarni yondiruvchi qudrat)",
        }
        grade_names = {"A": "🔥 A Toifa (Balerion/Drogon nasli)", "B": "💚 B Toifa (Rhaegal nasli)", "C": "💛 C Toifa (Viserion nasli)"}

        hunger_bar = "█" * (dragon.hunger // 10) + "░" * (10 - (dragon.hunger // 10))

        buttons = []
        if dragon.stage == "egg":
            buttons.append([InlineKeyboardButton("✨ Tuxumni Ochirish (1,500🌾 800⛓️ 500🪙)", callback_data="dragon_hatch")])
        else:
            buttons.append([InlineKeyboardButton("🍗 Boqish (350🌾)", callback_data="dragon_feed")])
            buttons.append([InlineKeyboardButton("🔥 Mashq Qildirish (300⛓️ 200🪙)", callback_data="dragon_train")])

        buttons.append([InlineKeyboardButton("🔙 Asosiy Menyu", callback_data="menu_main")])

        text = (
            f"🐉 **SIZNING AFSONAVIY AJDARINGIZ**\n\n"
            f"👑 Nomi: **{dragon.name}**\n"
            f"🏷️ Toifasi: **{grade_names.get(dragon.grade, dragon.grade)}**\n"
            f"🐣 Holati: **{stage_names.get(dragon.stage, dragon.stage)}**\n"
            f"🎖️ Daraja: **{dragon.level}-daraja**\n"
            f"⚔️ Olovli Jang Quvvati: **{dragon.power:,}**\n"
            f"🍖 To'qlik: `[{hunger_bar}]` **{dragon.hunger}%**\n\n"
            f"Ajdarlar qal'a qamallarida mudofaani yondirishda va dushman armiyasiga "
            f"katta talafot yetkazishda xizmat qiladi!"
        )
...
async def dragon_hatch_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not user:
            return
        ok, msg = await crud.hatch_dragon(session, user.id)
...
async def dragon_feed_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not user:
            return
        ok, msg = await crud.feed_dragon(session, user.id)
...
async def dragon_train_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not user:
            return
        ok, msg = await crud.train_dragon(session, user.id)
