...
async def show_champions_menu(target, user_id: int):
    ...
    async with AsyncSessionLocal() as session:
        ...
        text = (
            ...
            "Quyidagi sarkardalardan birini tanlang:"
        )
        ...
        for cid, cinfo in LEGENDARY_CHAMPIONS.items():
            ...
            buttons.append([
                InlineKeyboardButton(f"{cinfo['emoji']} {cinfo['name']}{status_tag}", callback_data=f"champ_info:{cid}")
            ])
            if not is_active:
                buttons.append([
...
async def champion_info_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"👑 **Armiyaga ta'siri:** Jang paytida ushbu qahramon armiya saflarida bo'lib, uning kuchi butun qo'shinga tarqaladi."
    )
